...
interface CurrencyLogosProps
  extends Omit<CurrencyLogoProps, "currency" | "size"> {
  currencies: Currency[];
  dense?: boolean;
  maxLogos?: number;
  size?: number;
...
const CurrencyLogoArray: FC<CurrencyLogosProps> = ({
  currencies,
  dense = false,
  maxLogos = 3,
  size = 24,
  ...
  const logos =
    currencies.length <= maxLogos
      ...
      : currencies.slice(
          0,
          Math.max(0, Math.min(currencies.length, maxLogos) - 1)
        );
    ...
    <div className="flex">
      ...
      {remnant > 0 && (
        <div
          ...
          style={{
            marginLeft:
              maxLogos > 1 ? (dense ? -Math.floor(size / 2 + 2) : -6) : 0,
            width: size,
            height: size,
            background:
