...
export type DepositAndApprovePayload = {
...
const useLimitOrderExecute: UseLimitOrderExecute = () => {
  ...
  const limitOrderContractAddress =
    chainId && STOP_LIMIT_ORDER_ADDRESS[chainId];
  ...
  const depositAndApprove = useCallback(
    async ({ inputAmount, bentoPermit }: DepositAndApprovePayload) => {
      ...
      try {
        const estimatedGas =
          await limitOrderHelperContract?.estimateGas.depositAndApprove(
            account,
  ...
  const deposit = useCallback<UseLimitOrderExecuteDeposit>(
    async ({ inputAmount, bentoPermit, fromBentoBalance }) => {
      if (!bentoBoxContract || !limitOrderContractAddress || !inputAmount)
      ...
      try {
        ...
        if (bentoPermit) {
          const { v, r, s } = bentoPermit;
          batch.push(
            bentoBoxContract.interface.encodeFunctionData(
              "setMasterContractApproval",
              [account, limitOrderContractAddress, true, v, r, s]
            )
          );
        }
  ...
  const execute = useCallback<UseLimitOrderExecuteExecute>(
    async ({ orderExpiration, inputAmount, outputAmount, recipient }) => {
      if (!inputAmount || !outputAmount || !account || !library)
        throw new Error("Dependencies unavailable");
