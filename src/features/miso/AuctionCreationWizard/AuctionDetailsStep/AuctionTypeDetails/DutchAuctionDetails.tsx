...
const DutchAuctionDetails: FC = () => {
  ...
  const paymentToken =
    useToken(data.paymentCurrencyAddress) ?? NATIVE[chainId || 1];
  ...
  if (paymentToken && auctionToken && Number(data.startPrice) > 0) {
    ...
    if (startBase && quote)
      startPrice = new Price({ baseAmount: quote, quoteAmount: startBase });
  ...
  if (paymentToken && auctionToken && Number(data.endPrice) > 0) {
    ...
    amount = tryParseAmount(data.tokenAmount?.toString(), auctionToken);
    if (endBase && quote)
      endPrice = new Price({ baseAmount: quote, quoteAmount: endBase });
    ...
    <>
      <div className="col-span-4 md:col-span-2">
        <Form.TextField
          ...
          placeholder={`0.00`}
          helperText={i18n._(
            t`The price when the auction will start. This value must be higher than the end price`
          )}
        />
      </div>
      <div className="col-span-4 md:col-span-2">
        <Form.TextField
          ...
          placeholder={`0.00`}
          helperText={i18n._(
            t`The price when the auction will meet its end date`
          )}
        />
      </div>
      <div className="col-span-4 md:col-span-2">
        ...
        <Typography className="mt-2">
          {minimumRaised ? minimumRaised.toSignificant(6) : "0.00"}{" "}
          {paymentToken?.symbol}
        </Typography>
        <FormFieldHelperText>
          {i18n._(
      ...
      <div className="col-span-4 md:col-span-2">
        ...
        <Typography className="mt-2">
          {maximumRaised ? maximumRaised?.toExact({}) : "0.00"}{" "}
          {paymentToken?.symbol}
        </Typography>
        <FormFieldHelperText>
          {i18n._(
            t`Maximum possible raised amount if all tokens were to be sold instantly at the starting price`
          )}
        </FormFieldHelperText>
      </div>
    </>
